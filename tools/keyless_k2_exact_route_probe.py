#!/usr/bin/env python3
"""Calibrate and hold out K2 v3 against the proven K1 route identity.

The candidate consumes already positioned Q plus raw V and derives route(V)
inside bounded N64 tiles. The oracle materializes the exact K1 row-route
arithmetic only on the diagnostic side, then runs dense attention with raw-V
retrieval. Holdout windows are disjoint from both v3 calibration and historical
K2-v2 holdout windows.
...
V3_HOLDOUT_CASES = (
    ("sixteenth-79x79", 1, 16, 79, 79),
    ("three-sixteenth-80x80", 3, 16, 80, 80),
    ("five-sixteenth-81x81", 5, 16, 81, 81),
    ("seven-sixteenth-159x159", 7, 16, 159, 159),
    ("nine-sixteenth-160x160", 9, 16, 160, 160),
    ("eleven-sixteenth-161x161", 11, 16, 161, 161),
    ("thirteen-sixteenth-319x321", 13, 16, 319, 321),
    ("fifteen-sixteenth-257x255", 15, 16, 257, 255),
    ("five-thirtysecond-513x385", 5, 32, 513, 385),
    ("twentyseven-thirtysecond-385x769", 27, 32, 385, 769),
)
