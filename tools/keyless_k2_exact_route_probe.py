...
PROBE_CONTRACT = "sol-h3-keyless-k2-exact-route-probe-v1"
...
def main() -> None:
    ...
    if source["tracked_worktree_dirty"]:
        parser.error("K2 exact-route calibration requires a clean tracked worktree")
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        parser.error("K2 exact-route calibration requires CUDA")
    if tuple(torch.cuda.get_device_capability(device)) != (12, 0):
        parser.error(
            "K2 exact-route calibration requires SM120, got "
    ...
    result = {
        ...
        "mode": "keyless-k2-exact-route-calibration",
        ...
        "limitations": [
            "threshold-free K2 v3 calibration only; no acceptance threshold is frozen",
