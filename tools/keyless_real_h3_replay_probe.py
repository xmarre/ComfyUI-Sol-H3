...
def main() -> None:
    ...
    parser.add_argument(
        "--expected-probe-commit",
        help=(
            "fail before loading the capture/checkpoint unless the local Sol-H3 "
            "checkout is exactly this git commit"
        ),
    )
    parser.add_argument(
        "--output-json",
    ...
    if args.expected_probe_commit:
        expected_probe_commit = args.expected_probe_commit.strip().lower()
        actual_probe_commit = str(probe_source["git_commit"])
        if actual_probe_commit != expected_probe_commit:
            parser.error(
                "stale Sol-H3 replay source: expected probe commit "
                f"{expected_probe_commit}, local checkout is {actual_probe_commit}; "
                "update the PR #21 mirror before running the expensive replay"
            )
        if bool(probe_source["tracked_worktree_dirty"]):
            parser.error(
                "Sol-H3 replay source has tracked local modifications; "
                "refusing an evidence run bound to a nominal commit"
            )
