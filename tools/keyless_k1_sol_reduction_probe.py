...
interchangeable oracle.  This probe tests a versioned K1 v3 candidate that:
...
from sol_h3.keyless_route_summary import (  # noqa: E402
    ...
    HEAD_DIM,
    NORM_EPS,
    SOL_REDUCTION_CONTRACT as K1_V3_CONTRACT,
    route_summary,
...
PROBE_CONTRACT = "sol-h3-keyless-k1-sol-reduction-probe-v1"
EVIDENCE_CONTRACT = "sol-h3-keyless-k1-sol-reduction-calibration-v1"
REQUIRED_K1_V2_CONTRACT = "sol-h3-keyless-route-summary-v2"
REQUIRED_K1_V3_CONTRACT = "sol-h3-keyless-route-summary-sol-reduction-v3"
...
def _run_block(
    ...
    *,
    ...
    for name, anchor, q_rows, v_rows, sink_start, sink_tokens in (
        ...
        v3_rc, v3_vc = route_summary_sol_reduction(
            v_raw,
            route_norm.to(device=device, dtype=torch.bfloat16),
            NORM_EPS,
            v_rope,
        )
        v3_rc = v3_rc.unsqueeze(0).contiguous()
        v3_vc = v3_vc.unsqueeze(0).contiguous()
        v3_threshold = threshold_from_route_centroids(
            qb,
            v3_rc,
            kv_rows=v_rows,
            tau=tau,
        ...
        variants = {
            ...
            "v3_selector_reference_vc": _variant(
                q=qb,
                route=rb,
                raw_v=vb,
                rc=v3_rc,
        ...
        cases.append(
            {
                ...
                "v3_candidate": {
                    "route_centroid": tensor_metrics(v3_rc, reference_rc),
                    "route_centroid_scale": tensor_scale_diagnostics(
                        v3_rc, reference_rc
                    ),
                    "raw_value_sum": tensor_metrics(v3_vc, reference_vc),
                    "threshold": tensor_metrics(v3_threshold, reference_threshold),
...
def _extrema(blocks: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "v3_candidate": {
            "route_centroid_rel_l2": max_metric(
                ("v3_candidate", "route_centroid"), "rel_l2"
            ),
            "raw_value_sum_rel_l2": max_metric(
                ("v3_candidate", "raw_value_sum"), "rel_l2"
            ),
            "raw_value_sum_max_abs": max_metric(
                ("v3_candidate", "raw_value_sum"), "max_abs"
            ),
            "threshold_rel_l2": max_metric(
                ("v3_candidate", "threshold"), "rel_l2"
            ),
            "all_allocations_below_full_route": all(
                bool(item["v3_candidate"]["allocation"]["below_full_route"])
                for item in flat
            ),
        },
        "v3_full": {
            "route_differing_bits": max_trace("v3_full", "differing_bits"),
            "route_differing_bit_fraction": max_trace(
                "v3_full", "differing_bit_fraction"
            ),
            "output_rel_l2": max_metric(
                ("variants", "v3_full", "output"), "rel_l2"
            ),
            "output_max_abs": max_metric(
                ("variants", "v3_full", "output"), "max_abs"
            ),
        },
        "decomposition": {
            name: {
...
def main() -> None:
    ...
    if K1_V3_CONTRACT != REQUIRED_K1_V3_CONTRACT:
        parser.error(f"unexpected K1 v3 contract {K1_V3_CONTRACT!r}")
    ...
    result = {
        ...
        "limitations": [
            "threshold-free recalibration only; no K1 v3 or K3 acceptance threshold is frozen",
            "exact-block K remains the materialized Comfy route while K1 reduction parity is isolated",
            "K1 v3 is experimental and not connected to production provider dispatch",
