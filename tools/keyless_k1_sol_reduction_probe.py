...
- spills one BF16 64-row route tile at a time and reloads it through the
  exact released _reduce_kv_kernel to isolate the final reduction boundary;
...
PROBE_CONTRACT = "sol-h3-keyless-k1-sol-reduction-probe-v7"
...
def _run_block(
    ...
    *,
    ...
    for name, anchor, q_rows, v_rows, sink_start, sink_tokens in (
        ...
        bounded_spill_rc, bounded_spill_scratch = (
            route_centroid_bounded_spill_diagnostic(
                v_raw,
                route_norm.to(device=device, dtype=torch.bfloat16),
                NORM_EPS,
        ...
        bounded_spill_rc = bounded_spill_rc.unsqueeze(0).contiguous()
        bounded_spill_reducer_config = _triton_config_identity(
            getattr(_reduce_kv_kernel, "best_config", None)
        )
        bounded_spill_threshold = threshold_from_route_centroids(
            qb,
            bounded_spill_rc,
            kv_rows=v_rows,
            tau=tau,
        ...
        bounded_spill_selector = _variant(
            q=qb,
            route=rb,
            raw_v=vb,
            rc=bounded_spill_rc,
            vc=reference_vc,
            threshold=bounded_spill_threshold,
            reference_output=reference_output,
            reference_trace=reference_trace,
            v_rows=v_rows,
            sink_start=sink_start,
        ...
        bounded_spill_isolation = {
            "reducer_config": bounded_spill_reducer_config,
            "rc_vs_materialized_existing_sol_rc": tensor_metrics(
                bounded_spill_rc,
                v5_materialized_rc,
            ),
            ...
            "threshold_vs_reference": tensor_metrics(
                bounded_spill_threshold,
                reference_threshold,
            ),
            "selector": bounded_spill_selector,
            **bounded_spill_scratch,
            "full_materialized_route_bytes": int(
                v_raw.numel() * v_raw.element_size()
            ),
            "max_working_scratch_fraction_of_full_route": (
                bounded_spill_scratch["max_working_scratch_bytes"]
                / int(v_raw.numel() * v_raw.element_size())
            ),
        }
        ...
        cases.append(
            {
                ...
                "bounded_spill_isolation": bounded_spill_isolation,
                "variants": variants,
            }
        )
    ...
    return {
        ...
        "bounded_spill_isolation": {
            ...
            "rc_vs_reference_rel_l2": max_metric(
                ("bounded_spill_isolation", "rc_vs_reference"),
                "rel_l2",
            ),
            "threshold_vs_reference_rel_l2": max_metric(
                ("bounded_spill_isolation", "threshold_vs_reference"),
                "rel_l2",
            ),
            ...
            "max_working_scratch_fraction_of_full_route": max(
                float(
                    item["bounded_spill_isolation"][
                        "max_working_scratch_fraction_of_full_route"
                    ]
                )
                for item in flat
            ),
            "reducer_configs": sorted(
                {
                    _config_key(
                        int(item["bounded_spill_isolation"]["reducer_config"]["num_warps"]),
                        int(item["bounded_spill_isolation"]["reducer_config"]["num_stages"]),
                    )
                    for item in flat
                }
            ),
...
def main() -> None:
    ...
    result = {
        ...
        "limitations": [
            ...
            "the bounded BF16 route-tile spill/reload is diagnostic only and is not a "
