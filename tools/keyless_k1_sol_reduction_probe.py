...
interchangeable oracle.  This probe confirms the fixed-b8 K1 v6 candidate while retaining v5 as a rejected control:

- derives v6 routed centroids from raw V through only bounded BF16 route chunks;
- reloads each bounded route chunk through the exact released Sol reducer;
- fixes the candidate chunk size at eight physical V64 blocks from v8 timing evidence;
- retains the released 4/8-warp x 1..4-stage matrix as a rejected-boundary control;
- retains the rejected split-RMS [T,H] diagnostic as a control;
- treats the proven one-block BF16 spill/reload as the exact arithmetic boundary;
- sweeps bounded multi-block spill chunks through the exact released
  _reduce_kv_kernel, recording RC/VC parity, scratch bytes and CUDA timing;
- reuses the exact released _reduce_kv_kernel for VC arithmetic;
- feeds v6 RC/VC into the unchanged SM120 Sol selector while v5 remains diagnostic only.
...
from sol_h3.keyless_route_summary import (  # noqa: E402
    BOUNDED_SPILL_CONTRACT as K1_BOUNDED_SPILL_CONTRACT,
    CONTRACT as K1_V2_CONTRACT,
    FUSED_V5_CONTROL_CONTRACT as K1_V5_CONTROL_CONTRACT,
    HEAD_DIM,
    NORM_EPS,
    SOL_REDUCTION_CHUNK_BLOCKS,
    SOL_REDUCTION_CONTRACT as K1_V6_CONTRACT,
...
PROBE_CONTRACT = "sol-h3-keyless-k1-sol-reduction-probe-v9"
...
REQUIRED_K1_V5_CONTROL_CONTRACT = (
...
HIDDEN = 5376
SCALE = HEAD_DIM ** -0.5
BOUNDED_SPILL_CHUNK_BLOCKS = (1, 2, 4, 8, 16)


def _triton_config_identity(config) -> dict[str, object]:
    if config is None:
        raise RuntimeError("authoritative Sol reducer did not expose an autotune best_config")
    return {
...
def _config_key(num_warps: int, num_stages: int) -> str:
    return f"w{int(num_warps)}-s{int(num_stages)}"


def _timing_ms(fn, device: torch.device, *, repeats: int = 3) -> dict[str, object]:
    warm = fn()
    del warm
    ...
    host_sorted = sorted(host_ms)
    middle = repeats // 2
    return {
        "repeats": repeats,
        "cuda_ms": {
            "min": min(cuda_ms),
            "median": cuda_sorted[middle],
            "max": max(cuda_ms),
        },
        "host_ms": {
            "min": min(host_ms),
            "median": host_sorted[middle],
            "max": max(host_ms),
        },
    }
...
def _run_block(
    ...
    *,
    ...
    from sol_h3._vendor.sol_attn.preprocess import _reduce_kv_kernel, prepare
    ...
    for name, anchor, q_rows, v_rows, sink_start, sink_tokens in (
        ...
        v5_rc, v5_vc = route_summary_sol_reduction_v5_diagnostic(
            v_raw,
            route_norm.to(device=device, dtype=torch.bfloat16),
            NORM_EPS,
            v_rope,
        ...
        v6_rc, v6_vc = route_summary_sol_reduction(
            v_raw,
            route_norm.to(device=device, dtype=torch.bfloat16),
            NORM_EPS,
            v_rope,
        )
        v6_rc = v6_rc.unsqueeze(0).contiguous()
        v6_vc = v6_vc.unsqueeze(0).contiguous()
        v6_threshold = threshold_from_route_centroids(
            qb,
            v6_rc,
            kv_rows=v_rows,
            tau=tau,
        ...
        authoritative_reducer_config = _triton_config_identity(
            getattr(_reduce_kv_kernel, "best_config", None)
        )
        authoritative_config_key = _config_key(
            int(authoritative_reducer_config["num_warps"]),
            int(authoritative_reducer_config["num_stages"]),
        )
        reduction_config_sweep: dict[str, object] = {}
        for config_warps in (4, 8):
            for config_stages in (1, 2, 3, 4):
                config_rc = route_centroid_sol_reduction_config_diagnostic(
                    v_raw,
                    route_norm.to(device=device, dtype=torch.bfloat16),
                    NORM_EPS,
                    v_rope,
                    num_warps=config_warps,
                    num_stages=config_stages,
                ).unsqueeze(0).contiguous()
                config_key = _config_key(config_warps, config_stages)
                reduction_config_sweep[config_key] = {
                    "num_warps": config_warps,
                    "num_stages": config_stages,
                    "matches_authoritative_best_config": (
                        config_key == authoritative_config_key
                    ),
                    "rc_vs_materialized_existing_sol_rc": tensor_metrics(
                        config_rc,
                        v5_materialized_rc,
                    ),
                    "rc_vs_reference": tensor_metrics(
                        config_rc,
                        reference_rc,
                    ),
                }
        ...
        split_rms_rc, split_rms_scratch_bytes = route_centroid_split_rms_diagnostic(
            v_raw,
            route_norm.to(device=device, dtype=torch.bfloat16),
            NORM_EPS,
            v_rope,
        )
        split_rms_rc = split_rms_rc.unsqueeze(0).contiguous()
        split_rms_threshold = threshold_from_route_centroids(
            qb,
            split_rms_rc,
            kv_rows=v_rows,
            tau=tau,
        ...
        split_rms_selector = _variant(
            q=qb,
            route=rb,
            raw_v=vb,
            rc=split_rms_rc,
            vc=reference_vc,
            threshold=split_rms_threshold,
            reference_output=reference_output,
            reference_trace=reference_trace,
            v_rows=v_rows,
            sink_start=sink_start,
        ...
        split_rms_layout_isolation = {
            ...
            "threshold_vs_reference": tensor_metrics(
                split_rms_threshold,
                reference_threshold,
            ),
            "selector": split_rms_selector,
            "scratch_bytes": split_rms_scratch_bytes,
            "full_materialized_route_bytes": int(
                v_raw.numel() * v_raw.element_size()
            ),
            "scratch_fraction_of_full_route": (
                split_rms_scratch_bytes
                / int(v_raw.numel() * v_raw.element_size())
            ),
        }

        full_route_bytes = int(v_raw.numel() * v_raw.element_size())
        bounded_chunk_sweep: dict[str, object] = {}
        for chunk_blocks in BOUNDED_SPILL_CHUNK_BLOCKS:
            chunk_rc, chunk_vc, chunk_scratch = route_summary_bounded_spill_diagnostic(
                v_raw,
                route_norm.to(device=device, dtype=torch.bfloat16),
                NORM_EPS,
                v_rope,
            ...
            chunk_rc = chunk_rc.unsqueeze(0).contiguous()
            chunk_vc = chunk_vc.unsqueeze(0).contiguous()
            chunk_threshold = threshold_from_route_centroids(
                qb,
                chunk_rc,
                kv_rows=v_rows,
                tau=tau,
            ...
            chunk_key = f"b{chunk_blocks}"
            timing = None
            if name.startswith("tail-"):
                timing = _timing_ms(
                    lambda chunk_blocks=chunk_blocks: (
                        route_summary_bounded_spill_diagnostic(
                            v_raw,
                            route_norm.to(device=device, dtype=torch.bfloat16),
                            NORM_EPS,
            ...
            bounded_chunk_sweep[chunk_key] = {
                "chunk_blocks": chunk_blocks,
                "route_centroid_vs_materialized_existing_sol": tensor_metrics(
                    chunk_rc,
                    v5_materialized_rc,
                ),
                "route_centroid_vs_reference": tensor_metrics(
                    chunk_rc,
                    reference_rc,
                ),
                "raw_value_sum_vs_reference": tensor_metrics(
                    chunk_vc,
                    reference_vc,
                ),
                "threshold_vs_reference": tensor_metrics(
                ...
                **chunk_scratch,
                ...
                "max_working_scratch_fraction_of_full_route": (
                    chunk_scratch["max_working_scratch_bytes"] / full_route_bytes
                ),
                "timing": timing,
            }

        v5_fused_timing = None
        if name.startswith("tail-"):
            v5_fused_timing = _timing_ms(
                lambda: route_summary_sol_reduction_v5_diagnostic(
                    v_raw,
                    route_norm.to(device=device, dtype=torch.bfloat16),
                    NORM_EPS,
                    v_rope,
        ...
        v5_reduction_identity = {
            "authoritative_reducer_config": authoritative_reducer_config,
            "authoritative_config_key": authoritative_config_key,
            "config_sweep": reduction_config_sweep,
        ...
        full_route_bytes = int(v_raw.numel() * v_raw.element_size())
        v5_allocation = replay_fixture._candidate_allocation(
            lambda: route_summary_sol_reduction_v5_diagnostic(
                v_raw,
                route_norm.to(device=device, dtype=torch.bfloat16),
                NORM_EPS,
        ...
        v5_allocation["full_materialized_route_bytes"] = full_route_bytes
        v5_allocation["below_full_route"] = (
            v5_allocation["temporary_peak_delta"] < full_route_bytes
        )

        v6_allocation = replay_fixture._candidate_allocation(
        ...
        v6_allocation["full_materialized_route_bytes"] = full_route_bytes
        v6_allocation["below_full_route"] = (
            v6_allocation["temporary_peak_delta"] < full_route_bytes
        )
        v6_route_scratch_bytes = int(
            min(v_rows, SOL_REDUCTION_CHUNK_BLOCKS * 64)
            * HEADS
            * HEAD_DIM
        ...
        cases.append(
            {
                ...
                "v5_candidate": {
                    ...
                    "allocation": v5_allocation,
                },
                "v6_candidate": {
                    "contract": K1_V6_CONTRACT,
                    "chunk_blocks": SOL_REDUCTION_CHUNK_BLOCKS,
                    "route_scratch_bytes": v6_route_scratch_bytes,
                    "route_scratch_fraction_of_full_route": (
                        v6_route_scratch_bytes / full_route_bytes
                    ),
                    "route_centroid_vs_materialized_existing_sol": tensor_metrics(
                        v6_rc, v5_materialized_rc
                    ),
                    "route_centroid_vs_reference": tensor_metrics(v6_rc, reference_rc),
                    "raw_value_sum_vs_reference": tensor_metrics(v6_vc, reference_vc),
                    "threshold_vs_reference": tensor_metrics(
                        v6_threshold, reference_threshold
                    ),
                    "selector": v6_selector,
                    "allocation": v6_allocation,
                    "timing": v6_timing,
                },
                "v5_reduction_identity": v5_reduction_identity,
                "split_rms_layout_isolation": split_rms_layout_isolation,
                "bounded_chunk_sweep": bounded_chunk_sweep,
                "v5_fused_timing_control": v5_fused_timing,
                "variants": variants,
            }
        )
...
def _extrema(blocks: list[dict[str, object]]) -> dict[str, object]:
    ...
    config_keys = tuple(
        _config_key(num_warps, num_stages)
        for num_warps in (4, 8)
        for num_stages in (1, 2, 3, 4)
    )

    def config_max(config_key: str) -> dict[str, object]:
        row = max(
            flat,
            key=lambda item: float(
                item["v5_reduction_identity"]["config_sweep"][config_key][
                    "rc_vs_materialized_existing_sol_rc"
                ]["rel_l2"]
            ),
        )
        metric = row["v5_reduction_identity"]["config_sweep"][config_key][
            "rc_vs_materialized_existing_sol_rc"
        ]
        return {
            "value": float(metric["rel_l2"]),
            "block_index": int(row["block_index"]),
            "case": str(row["name"]),
        }
    ...
    for item in flat:
        config_key = str(
            item["v5_reduction_identity"]["authoritative_config_key"]
        )
        metric = item["v5_reduction_identity"]["config_sweep"][config_key][
            "rc_vs_materialized_existing_sol_rc"
        ]
        authoritative_rows.append((item, config_key, metric))
    worst_authoritative = max(
        authoritative_rows,
        key=lambda entry: float(entry[2]["rel_l2"]),
    )
    authoritative_configs = sorted(
        {
            str(item["v5_reduction_identity"]["authoritative_config_key"])
            for item in flat
        }
    )
    ...
    return {
        ...
        "v6_candidate": {
            "route_centroid_vs_materialized_existing_sol_rel_l2": max_metric(
                ("v6_candidate", "route_centroid_vs_materialized_existing_sol"),
                "rel_l2",
            ),
            "route_centroid_vs_reference_rel_l2": max_metric(
                ("v6_candidate", "route_centroid_vs_reference"),
                "rel_l2",
            ),
            "raw_value_sum_vs_reference_rel_l2": max_metric(
                ("v6_candidate", "raw_value_sum_vs_reference"),
                "rel_l2",
            ),
            "threshold_vs_reference_rel_l2": max_metric(
                ("v6_candidate", "threshold_vs_reference"),
                "rel_l2",
            ),
            "selector_route_differing_bits": max_metric(
                ("v6_candidate", "selector", "route_trace"),
                "differing_bits",
            ),
            "selector_output_rel_l2": max_metric(
                ("v6_candidate", "selector", "output"),
                "rel_l2",
            ),
            "all_allocations_below_full_route": all(
                bool(item["v6_candidate"]["allocation"]["below_full_route"])
                for item in flat
            ),
            "max_route_scratch_bytes": max(
                int(item["v6_candidate"]["route_scratch_bytes"])
                for item in flat
            ),
            "max_route_scratch_fraction_of_full_route": max(
                float(item["v6_candidate"]["route_scratch_fraction_of_full_route"])
                for item in flat
            ),
            "tail_timing_cuda_ms": [
                {
                    ...
                    "case": str(item["name"]),
                    **item["v6_candidate"]["timing"]["cuda_ms"],
                }
                for item in flat
                if item["v6_candidate"]["timing"] is not None
            ],
        },
        "v5_candidate": {
            "route_centroid_rel_l2": max_metric(
        ...
        "v5_reduction_identity": {
            "authoritative_reducer_configs": authoritative_configs,
            "config_sweep_rc_vs_materialized_existing_sol_rc_rel_l2": {
                config_key: config_max(config_key)
                for config_key in config_keys
            },
            "authoritative_config_rc_vs_materialized_existing_sol_rc_rel_l2": {
                "value": float(worst_authoritative[2]["rel_l2"]),
                "block_index": int(worst_authoritative[0]["block_index"]),
                "case": str(worst_authoritative[0]["name"]),
                "config": str(worst_authoritative[1]),
            },
            ...
            chunk_key: {
                "route_centroid_vs_materialized_existing_sol_rel_l2": max_metric(
                    (
                        "bounded_chunk_sweep",
                        chunk_key,
                        "route_centroid_vs_materialized_existing_sol",
                    ),
                    "rel_l2",
                ),
                "raw_value_sum_vs_reference_rel_l2": max_metric(
                ...
                "max_working_scratch_fraction_of_full_route": max(
                    float(
                        item["bounded_chunk_sweep"][chunk_key][
                            "max_working_scratch_fraction_of_full_route"
                        ]
                    )
                    for item in flat
                ...
                "tail_timing_cuda_ms": [
                    {
                        "block_index": int(item["block_index"]),
                        "case": str(item["name"]),
                        **item["bounded_chunk_sweep"][chunk_key]["timing"]["cuda_ms"],
                    }
                    for item in flat
                    if item["bounded_chunk_sweep"][chunk_key]["timing"] is not None
                ],
            }
            for chunk_key in tuple(
                f"b{chunk_blocks}"
                for chunk_blocks in BOUNDED_SPILL_CHUNK_BLOCKS
            )
        },
        "v5_fused_timing_control": [
            {
                "block_index": int(item["block_index"]),
                "case": str(item["name"]),
                **item["v5_fused_timing_control"]["cuda_ms"],
            }
            for item in flat
            if item["v5_fused_timing_control"] is not None
        ],
        "split_rms_layout_isolation": {
            "rc_vs_materialized_existing_sol_rc_rel_l2": max_metric(
                (
                    "split_rms_layout_isolation",
                    "rc_vs_materialized_existing_sol_rc",
                ),
            ...
            "rc_vs_reference_rel_l2": max_metric(
                ("split_rms_layout_isolation", "rc_vs_reference"),
                "rel_l2",
            ),
            "threshold_vs_reference_rel_l2": max_metric(
                ("split_rms_layout_isolation", "threshold_vs_reference"),
                "rel_l2",
            ),
            "selector_route_differing_bits": max_metric(
                (
            ...
            "max_scratch_fraction_of_full_route": max(
                float(item["split_rms_layout_isolation"]["scratch_fraction_of_full_route"])
                for item in flat
            ),
        },
        "v5_full": {
...
def main() -> None:
    ...
    if K1_V6_CONTRACT != REQUIRED_K1_V6_CONTRACT:
        parser.error(f"unexpected K1 v6 contract {K1_V6_CONTRACT!r}")
    if K1_V5_CONTROL_CONTRACT != REQUIRED_K1_V5_CONTROL_CONTRACT:
        parser.error(
            f"unexpected K1 v5 control contract {K1_V5_CONTROL_CONTRACT!r}"
        )
    if SOL_REDUCTION_CHUNK_BLOCKS != 8:
        parser.error(
            f"unexpected K1 v6 fixed chunk size {SOL_REDUCTION_CHUNK_BLOCKS!r}"
        )
    if K1_BOUNDED_SPILL_CONTRACT != REQUIRED_K1_BOUNDED_SPILL_CONTRACT:
        parser.error(
            f"unexpected K1 bounded-spill contract {K1_BOUNDED_SPILL_CONTRACT!r}"
        )
    ...
    result = {
        ...
        "source_contracts": {
            ...
            "k1_v6": K1_V6_CONTRACT,
            "k1_v5_control": K1_V5_CONTROL_CONTRACT,
            "k1_bounded_spill": K1_BOUNDED_SPILL_CONTRACT,
            "k3": K3_CONTRACT,
        },
        "evidence_lineage": {
            "prior_k1_v8_bounded_chunk_sha256": (
                "23d02bc221dba598585af3c32834e89a79ec8a69a74e0deb7a43e51639e9488c"
            ),
            "prior_k1_v7_bounded_spill_sha256": (
                "09ffdabc9b7872bd6402ea643429f75045b36bf5a4b7f8a3d9c6219a3c486b30"
            ),
            "prior_k1_v6_split_rms_sha256": (
                "a4958a2e5ea13a22a513cd09809addeeeac4479966e71cd4d612b5ffbb616735"
            ),
            "prior_k1_v5_launch_config_sha256": (
                "aebd10697b052a4dab640209a5ce21d5aa25debb6f869f6a4675c5710a1be4a2"
            ),
            "prior_k1_v5_calibration_sha256": (
                "b255b10a6e226a60bd229926e02cd04957ae41dcebba35d3f23e943680dc7706"
            ),
        ...
        "limitations": [
            "threshold-free recalibration only; no K1 v6 or K3 acceptance threshold is frozen",
            ...
            "the released-Sol 4/8-warp x 1..4-stage launch matrix is a rejected-boundary "
            "control and does not select a production config policy",
            "the split-RMS [T,H] FP32 scratch is a rejected-boundary control",
            "v8 established exact parity for b1/b2/b4/b8/b16 and selected b8 as the "
            "bounded K1 candidate from memory/timing evidence",
            "v9 records seven-repeat candidate timing on the frozen tail geometry for "
            "each early/mid/late block",
