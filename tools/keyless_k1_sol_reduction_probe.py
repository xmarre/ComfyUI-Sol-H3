...
interchangeable oracle.  This probe tests a versioned K1 v4 candidate that:
...
- then feeds the v4 RC/VC into the unchanged SM120 Sol selector.
...
from sol_h3.keyless_route_summary import (  # noqa: E402
    ...
    HEAD_DIM,
    NORM_EPS,
    SOL_REDUCTION_CONTRACT as K1_V4_CONTRACT,
...
PROBE_CONTRACT = "sol-h3-keyless-k1-sol-reduction-probe-v3"
...
def _run_block(
    ...
    *,
    ...
    for name, anchor, q_rows, v_rows, sink_start, sink_tokens in (
        ...
        v4_rc, v4_vc = route_summary_sol_reduction(
            v_raw,
            route_norm.to(device=device, dtype=torch.bfloat16),
            NORM_EPS,
            v_rope,
        )
        v4_rc = v4_rc.unsqueeze(0).contiguous()
        v4_vc = v4_vc.unsqueeze(0).contiguous()
        v4_threshold = threshold_from_route_centroids(
            qb,
            v4_rc,
            kv_rows=v_rows,
            tau=tau,
        ...
        # Diagnostic-only full-route materialization. This is not an implementation
        # candidate: it exists solely to distinguish row-route arithmetic from the
        # fused RC reduction boundary using the exact existing Sol reducer.
        v4_materialized_route = materialized_sol_reduction_v4_route_diagnostic(
            v_raw,
            route_norm.to(device=device, dtype=torch.bfloat16),
            NORM_EPS,
            v_rope,
        )
        v4_materialized_route_b = v4_materialized_route.unsqueeze(0).contiguous()
        (
            v4_materialized_rc,
            v4_materialized_vc,
            v4_materialized_threshold,
        ) = prepare(
            qb,
            v4_materialized_route_b,
            vb,
            tau=tau,
            scale=SCALE,
            thresh_type="diag",
        ...
        v4_materialized_selector = _variant(
            q=qb,
            route=rb,
            raw_v=vb,
            rc=v4_materialized_rc,
            vc=v4_materialized_vc,
            threshold=v4_materialized_threshold,
        ...
        v4_reduction_isolation = {
            "row_route": tensor_metrics(v4_materialized_route_b, rb),
            ...
            "selector": v4_materialized_selector,
        ...
        variants = {
            ...
            "v4_selector_reference_vc": _variant(
                q=qb,
                route=rb,
                raw_v=vb,
                rc=v4_rc,
        ...
        cases.append(
            {
                ...
                "v4_candidate": {
                    "route_centroid": tensor_metrics(v4_rc, reference_rc),
                    "route_centroid_scale": tensor_scale_diagnostics(
                        v4_rc, reference_rc
                    ),
                    "raw_value_sum": tensor_metrics(v4_vc, reference_vc),
                    "threshold": tensor_metrics(v4_threshold, reference_threshold),
                    "allocation": allocation,
                },
                "v4_reduction_isolation": v4_reduction_isolation,
                "variants": variants,
            }
        )
...
def _extrema(blocks: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "v4_candidate": {
            "route_centroid_rel_l2": max_metric(
                ("v4_candidate", "route_centroid"), "rel_l2"
            ),
            "raw_value_sum_rel_l2": max_metric(
                ("v4_candidate", "raw_value_sum"), "rel_l2"
            ),
            "raw_value_sum_max_abs": max_metric(
                ("v4_candidate", "raw_value_sum"), "max_abs"
            ),
            "threshold_rel_l2": max_metric(
                ("v4_candidate", "threshold"), "rel_l2"
            ),
            "all_allocations_below_full_route": all(
                bool(item["v4_candidate"]["allocation"]["below_full_route"])
                for item in flat
            ),
        },
        ...
        "v4_reduction_isolation": {
            "row_route_rel_l2": max_metric(
                ("v4_reduction_isolation", "row_route"),
                "rel_l2",
            ),
            "materialized_existing_sol_rc_rel_l2": max_metric(
                ("v4_reduction_isolation", "materialized_existing_sol_rc"),
                "rel_l2",
            ),
            ...
            "materialized_threshold_rel_l2": max_metric(
                ("v4_reduction_isolation", "materialized_threshold"),
                "rel_l2",
            ),
        ...
        "v4_full": {
            "route_differing_bits": max_trace("v4_full", "differing_bits"),
            "route_differing_bit_fraction": max_trace(
                "v4_full", "differing_bit_fraction"
            ),
            "output_rel_l2": max_metric(
                ("variants", "v4_full", "output"), "rel_l2"
            ),
            "output_max_abs": max_metric(
                ("variants", "v4_full", "output"), "max_abs"
            ),
        },
        "decomposition": {
            name: {
...
def main() -> None:
    ...
    if K1_V4_CONTRACT != REQUIRED_K1_V4_CONTRACT:
        parser.error(f"unexpected K1 v4 contract {K1_V4_CONTRACT!r}")
    ...
    result = {
        ...
        "source_contracts": {
            ...
            "k1_v4": K1_V4_CONTRACT,
            "k3": K3_CONTRACT,
        },
        "evidence_lineage": {
            "prior_k1_v3_calibration_sha256": "435f1ac38921bc46f4cd1efe58fff5b79439d928e7f58c1d128cfe2bb3464345",
            "prior_k3_calibration_sha256": "8017876a937fe8a25287a231ccf8a331422514f36241a80675d52ef39a96d71b",
        },
        ...
        "limitations": [
            "threshold-free recalibration only; no K1 v4 or K3 acceptance threshold is frozen",
            "exact-block K remains the materialized Comfy route while K1 reduction parity is isolated",
            "a full K1-v4 route is materialized only in the diagnostic/oracle path to isolate row-route arithmetic from fused reduction",
            ...
            "no fused selected-route CuTe production mainloop, decoded-media, sampler, or end-to-end performance evidence",
        ],
    }
