"""SM120 synthetic SOL + optional inherited-dense probe; does not evaluate media quality."""
import argparse
import json
...
def main():
    ...
    def requested_dense(original, q, k, v, heads, **kw):
        if args.backend == "sage":
            ...
            return pytorch_dense(original, q, k, v, heads, **kw)
        return out if kw.get("skip_output_reshape") else out.transpose(1, 2).reshape(1, q.shape[2], -1)
    ...
    def block(call_args):
        ...
        result = options["optimized_attention_override"](
            pytorch_dense, q, k, v, args.heads,
            skip_reshape=True, transformer_options=options, _inside_attn_wrapper=True,
        )
        assert result.shape == (1, args.tokens, args.heads * 128)
        reference_provider = pytorch_dense if state.dense_provider_failures else requested_dense
        reference = reference_provider(None, q[:, :, :args.prefix], k, v, args.heads, skip_output_reshape=False)
    ...
    with torch.no_grad():
        for _ in range(2):
            ...
            try:
                BlockPatch(0, config)(
                    {"img": q.new_empty(args.tokens, args.heads * 128),
                     "layout": layout,
                     "transformer_options": {"optimized_attention_override": requested_dense}},
    ...
    print(json.dumps({
        "requested_dense_backend": args.backend,
        "effective_dense_backends": sorted(state.dense_attention_backends),
        "dense_provider_failures": dict(state.dense_provider_failures),
        "dense_provider_fallback": bool(state.dense_provider_failures),
        "gpu": torch.cuda.get_device_name(),
        **config.metadata(),
        "sol_backend": state.kernel.backend_name,
