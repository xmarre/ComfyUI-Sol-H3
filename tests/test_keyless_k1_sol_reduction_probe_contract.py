...
def test_k1_sol_reduction_probe_contracts_are_versioned():
    ...
    assert 'PROBE_CONTRACT = "sol-h3-keyless-k1-sol-reduction-probe-v1"' in source
    assert (
...
def test_k1_sol_reduction_probe_compares_v2_v3_and_existing_sol_reduction():
    source = _probe_source()
    assert "reference_rc, reference_vc, reference_threshold = prepare(" in source
    assert "v2_rc, v2_vc = route_summary(" in source
    assert "v3_rc, v3_vc = route_summary_sol_reduction(" in source
    assert "tensor_metrics(v3_rc, reference_rc)" in source
    assert "tensor_metrics(v3_vc, reference_vc)" in source
