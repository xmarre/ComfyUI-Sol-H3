...
def test_sol_reduction_k1_rc_keeps_route_row_local_before_reduction():
    source = _source()
    assert "mean_square = tl.sum(raw * raw, axis=1) / D" in source
    assert "route = tl.where(" in source
    assert "summary = tl.sum(route.to(tl.float32), axis=0) / block_len" in source
    assert "lane_offsets = tl.arange(0, 32)" in source
    assert "lane_sum = tl.fma(lane3, lane3, lane_sum)" in source
    assert "mean_square = tl.sum(lane_sum, axis=1) / D" in source
    assert "torch.empty_like(v)" not in source[source.index("def route_summary_sol_reduction"):source.index("def route_summary(")]
