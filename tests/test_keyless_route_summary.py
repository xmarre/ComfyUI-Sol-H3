...
def _direct_route(v, weight, eps, rope):
    ...
    a = norm[..., : summary.ROPE_HALF_DIM].float()
    b = norm[..., summary.ROPE_HALF_DIM : summary.ROPE_ROT_DIM].float()
    rotated = torch.cat(
        (a * c - b * s, a * s + b * c),
        dim=-1,
    ).to(v.dtype)
    return torch.cat(
        (rotated, norm[..., summary.ROPE_ROT_DIM :]),
        dim=-1,
    ), norm
...
def test_materialized_fp32_rope_route_diagnostic_fails_closed_without_cuda():
    v = torch.ones(3, 2, summary.HEAD_DIM, dtype=torch.bfloat16)
    weight = torch.ones(summary.HEAD_DIM, dtype=torch.bfloat16)
    rope = _rope(v.shape[0])
    with pytest.raises(RuntimeError, match="requires CUDA"):
        summary.materialized_fp32_rope_route_diagnostic(
            v,
            weight,
            summary.NORM_EPS,
            rope,
...
def test_fp32_rope_diagnostic_keeps_comfy_fused_compute_semantics_in_source():
    source = Path(summary.__file__).read_text(encoding="utf-8")
    assert "rope_compute_fp32: tl.constexpr" in source
    assert "norm_math = norm.to(tl.float32)" in source
    assert "partner_math = partner_norm.to(tl.float32)" in source
    assert "cos_values = cos_loaded.to(tl.float32)" in source
    assert "sin_values = sin_loaded.to(tl.float32)" in source


def test_route_reference_fp32_rotation_differs_from_bf16_operation_boundaries():
    # This tuple is deliberately chosen so FP32 multiply/add followed by one
    # BF16 cast differs from materializing the individual BF16 products.
    v = torch.ones(1, 1, summary.HEAD_DIM, dtype=torch.bfloat16)
    weight = torch.full(
        (summary.HEAD_DIM,),
        0.10009765625,
        dtype=torch.bfloat16,
    )
    rope = torch.zeros(1, 1, 1, summary.ROPE_HALF_DIM, 2, 2, dtype=torch.bfloat16)
    rope[..., 0, 0] = torch.tensor(0.10009765625, dtype=torch.bfloat16)
    rope[..., 0, 1] = torch.tensor(-0.69921875, dtype=torch.bfloat16)
    rope[..., 1, 0] = torch.tensor(0.69921875, dtype=torch.bfloat16)
    rope[..., 1, 1] = torch.tensor(0.10009765625, dtype=torch.bfloat16)

    got = summary.materialized_route_reference(v, weight, summary.NORM_EPS, rope)
    work = v.float()
    inv = torch.rsqrt(work.square().mean(dim=-1, keepdim=True) + summary.NORM_EPS)
    norm = (work * inv * weight.float()).to(torch.bfloat16)
    c = rope[0, :, 0, :, 0, 0].unsqueeze(1)
    s = rope[0, :, 0, :, 1, 0].unsqueeze(1)
    a = norm[..., :summary.ROPE_HALF_DIM]
    b = norm[..., summary.ROPE_HALF_DIM:summary.ROPE_ROT_DIM]
    bf16_boundary = torch.cat(
        (a * c - b * s, a * s + b * c, norm[..., summary.ROPE_ROT_DIM:]),
        dim=-1,
    )
    assert not torch.equal(got, bf16_boundary)
