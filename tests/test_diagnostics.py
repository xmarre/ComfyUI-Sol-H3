...
def test_multiple_boundary_drains_resolve_only_new_samples():
    syncs = []
    state = CudaDiagnosticState(
        enabled=True,
        max_samples=4,
    ...
    first = state.begin_sample("production", "cuda:0")
    with state.span(first, "production_call"):
        pass
    state.drain_pending()
