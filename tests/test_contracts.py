...
def test_identity_changes_with_every_numerical_option():
    ...
    assert base["backend"] == "inherit"
    assert base["attention_ownership"] == "inherit"
    assert base["approximate"] is False
    assert base["kernel_contract"] is None and base["sink_mode"] is None and base["threshold"] is None
    sparse = Config(backend="sol").metadata()
    assert sparse["attention_ownership"] == "sol" and sparse["kernel_contract"] is not None
    base["tau"] = 99
    assert Config().metadata()["tau"] == 1


@pytest.mark.parametrize("kwargs", [{"backend": "sol_bsa"}, {"backend": "auto"}, {"backend": "dense"},
