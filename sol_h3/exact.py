...
def affine(h, shift, scale, segments, verified):
    ...
    for a, b, row in segments:
        ...
        if b == a:
            continue
        # Re-gate every distinct physical layout, not only dtype/width. The kernel
        # consumes explicit strides, and a later provider is allowed to change them.
        key = (h.device, h.dtype, shift.dtype, scale.dtype, h.shape[1], indexed,
               h.stride(0), tuple(shift.stride()), tuple(scale.stride()))
        want = None
...
def execute_block(block, args, verified):
    ...
    global_forward_hooks = getattr(module_hooks, "_global_forward_hooks", {})
    global_forward_pre_hooks = getattr(module_hooks, "_global_forward_pre_hooks", {})
    if (block._forward_hooks or block._forward_pre_hooks or global_forward_hooks or global_forward_pre_hooks
            or getattr(block, "_compiled_call_impl", None) is not None):
        raise RuntimeError("Exact fusion cannot bypass block/global forward hooks or compiled block calls; disable exact fusion")
