...
ROPE_ROT_DIM = 96
NORM_EPS = 1e-5
CONTRACT = "sol-h3-keyless-route-summary-v1"
...
if triton is not None:
    ...
    def _reduce_route_vc_kernel(
        ...
        first = norm * cos_values - partner_norm * sin_values
        second = partner_norm * sin_values + norm * cos_values
        route = tl.where(
            first_half[None, :],
            first,
    ...
    def _materialize_native_route_kernel(
        ...
        norm_cos = norm * cos_values
        partner_sin = partner_norm * sin_values
        norm_sin = norm * sin_values
        partner_cos = partner_norm * cos_values
...
def materialized_route_reference(
    ...
    """Torch oracle matching the public Keyless RMSNorm + split-half RoPE order."""
    ...
    rot = rope_freqs.to(device=v.device, dtype=v.dtype)
    c = rot[0, :, 0, :, 0, 0].unsqueeze(1)
    s = rot[0, :, 0, :, 1, 0].unsqueeze(1)
    a = norm[..., :ROPE_HALF_DIM]
    b = norm[..., ROPE_HALF_DIM:ROPE_ROT_DIM]
    return torch.cat(
        (
            a * c - b * s,
            a * s + b * c,
