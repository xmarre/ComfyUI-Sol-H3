...
def route_centroid_split_rms_diagnostic(
    ...
    Diagnostic only. The first kernel computes the exact current per-row/head
    inverse RMS into [T,H] FP32 storage. The second kernel reloads that scalar,
    ...
    _compute_route_inv_rms_kernel[(blocks, heads)](
        v,
        inv_rms,
        rows,
        v.stride(0),
...
def route_summary_sol_reduction(
    ...
    """K1 v5 candidate using the released-Sol BF16 reduction expression.
