...
SOL_REDUCTION_CONTRACT = "sol-h3-keyless-route-summary-sol-reduction-v4"
...
if triton is not None:
    ...
    def _reduce_route_rc_sol_kernel(
        ...
        summary = tl.sum(route.to(tl.float32), axis=0) / block_len
...
def route_summary_sol_reduction(
    ...
    """K1 v4 candidate aligned to existing Sol reduction and Comfy RMS order.

    RC is derived from raw V with the released Sana block-reduction geometry and
    the D=128 comfy-kitchen CUDA RMS lane/FMA ordering. VC is produced by the exact pinned
